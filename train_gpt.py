...
class Hyperparameters:
    ...
    bigram_vocab_size = int(os.environ.get("BIGRAM_VOCAB_SIZE", 7168))
...
class CausalSelfAttention(nn.Module):
    ...
    def _xsa_efficient(self, y: Tensor, v: Tensor) -> Tensor:
        ...
        vn = F.normalize(v.transpose(1, 2), dim=-1).unsqueeze(-2)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if _USE_FA3:
            ...
            k = F.rms_norm(k, (k.size(-1),))
            cos, sin = self.rotary(seqlen, x.device, q.dtype)
            cos_fa = cos.permute(0, 2, 1, 3)
            sin_fa = sin.permute(0, 2, 1, 3)
            q = apply_rotary_emb(q, cos_fa, sin_fa)
            k = apply_rotary_emb(k, cos_fa, sin_fa)
            q = q * self.q_gain.to(dtype=q.dtype)[None, None, :, None]
...
def ttt_adapt(
    ...
    """Full-weight TTT: SGD adaptation on val data."""
    seq_len = args.train_seq_len
...
def main() -> None:
    ...
    if args.ttt_enabled:
        log0("ttt:starting test-time training")
        ttt_adapt(args, base_model, device, val_tokens, rank, world_size, log_fn=log0)
